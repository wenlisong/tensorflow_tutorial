...
# CNN, but deConv is not finished
# image = tf.reshape(tf_x, [-1, 28, 28, 1])
# conv0 = tf.layers.conv2d(image, 16, 3, 1, 'same', activation=tf.nn.tanh)  # 28,28,16
# pool0 = tf.layers.max_pooling2d(conv0, 2, 2)  # 14,14,16
# conv1 = tf.layers.conv2d(pool0, 32, 3, 1, 'same', activation=tf.nn.tanh)  # 14,14,32
# pool1 = tf.layers.max_pooling2d(conv1, 2, 2)  # 7,7,32
# pool1 = tf.reshape(pool1, [-1, 7 * 7 * 32])
# en2 = tf.layers.dense(pool1, 10, tf.nn.tanh)

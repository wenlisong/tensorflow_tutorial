...
accuracy, acc_op = tf.metrics.accuracy(labels=tf.argmax(test_y, axis=1), predictions=tf.argmax(output, axis=1))
...
with tf.Session() as sess:
    ...
    for step in range(500):
        ...
        if step % 100 == 0:
            accuracy_, flat_representation = sess.run([acc_op, flatten], {tf_x: test_x, tf_y: test_y})
